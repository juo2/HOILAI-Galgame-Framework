...
namespace XGUI
{
    public class XListView : XView, XIEvent
    {
        ...
        public bool customNotRecalculate = false;
        ...
        private ListEventLua m_OnDataUpdateComplete = new ListEventLua();
        ...
        public int waitCreateCount { get { return m_WaitCreateCount; } set { m_WaitCreateCount = value; } }
        ...
        [SerializeField]
        private bool m_AnchorsAutoChange = true;
        ...
        private List<int> m_WaitFrameUpdateIndexList;  //分帧刷新
        ...
        [SerializeField]
        public List<GameObject> preCreateItemGameObject;
        ...
        public int GridConstraintCount { get { return m_GridConstraintCount; } }
        ...
        public ListEventLua onDataUpdateComplete { get { return m_OnDataUpdateComplete; } }
        ...
        void Initialize()
        {
            ...
            else
            {
                ...
                if (this.m_Content == null)
                    this.m_Content = GetComponent<RectTransform>();
            }
            ...
            if (m_ListLayout == ListLayout.Custom && this.m_Content != null)
            {
                ...
                if (this.m_Content.childCount > 0)
                {
                    ...
                    {
                        if (!item.name.StartsWith("Ignore") && item.gameObject != m_Template)
                            m_CustomParents.Add(item);
                    }
                }
            }

            if (!string.IsNullOrEmpty(m_TemplateAsset) && !m_Template)
                LoadTemplateAsset();
        }
        ...
        public void Update()
        {
            if (m_Template == null || customNotRecalculate)
                return;
            ...
            if (this.m_Recalculate)
            {
                ...
                //if (m_DataCount == 0 || m_WaitCreateCount <= 0) //无分帧创建直接调用OnUpdatePost
                //    UpdatePost();
            }
            ...
            if (m_WaitCreateList != null && m_WaitCreateList.Count > 0)
            {
                ...
                if (m_WaitCreateList.Count == 0) //分帧创建完毕调用OnUpdatePost
                    UpdatePost();
            }

            if (m_WaitFrameUpdateIndexList != null && m_WaitFrameUpdateIndexList.Count > 0)
            {
                //分帧调用lua中的on_data
                int index = -1;
                int callCount = 0;
                ListItemRenderer renderer = null;
                while (m_WaitFrameUpdateIndexList.Count > 0)
                {
                    index = m_WaitFrameUpdateIndexList[0];
                    m_WaitFrameUpdateIndexList.RemoveAt(0);

                    if (!m_ListItems.ContainsKey(index) && m_WaitCreateList.ContainsKey(index))
                    {
                        //还在等待创建，那就先创建了
                        UpdateWiatCreateItem(index, m_WaitCreateList[index]);
                        m_WaitCreateList.Remove(index);
                    }


                    if (m_ListItems.TryGetValue(index, out renderer))
                    {
                        ++callCount;
                        InvokeUpdateEvent(renderer, true, false);
                        if (m_WaitFrameUpdateIndexList.Count == 0 && m_WaitCreateList.Count == 0)
                            m_OnDataUpdateComplete.Invoke(renderer.instanceID);
                    }

                    if (callCount >= m_WaitCreateCount)
                        break;
                }
        ...
        protected void UpdateCustomLayout()
        {
            ...
            int starIndex = !customNotRecalculate ? 0 : lastListCount;

            for (int i = starIndex; i < m_DataCount; i++)  //单向插入
            {
            ...
            this.m_Recalculate = customNotRecalculate ? false : this.m_Recalculate;
            customNotRecalculate = customNotRecalculate ? !customNotRecalculate : customNotRecalculate;
        }
        ...
        protected void Recycle(int idx)
        {
            ...
            {
                render.recyclePos = render.transform.localPosition;
        ...
        protected void CreateItemRenderer(int index, Vector2 offset, out ListItemRenderer render)
        {
            ...
            if (this.m_RecyclePool != null && this.m_RecyclePool.Count > 0)
            {
                ...
                if (m_ListLayout == ListLayout.Custom)
                    render.transform.localPosition = render.recyclePos;
                InvokeUpdateEvent(render, true);
            ...
            //{
            //    render = new ListItemRenderer();
            //    render.SetData(index, CreateItemGameObject(this.m_Template, GetCreateItemParent(index)));
        ...
        void OnItemRendererCreate(int index, Vector2 offset, ListItemRenderer render)
        {
            ...
            if (this.m_ListLayout != ListLayout.Custom && m_AnchorsAutoChange)
            {
        ...
        protected void UpdateWiatCreateItem(int index, ListItemRenderer render)
        {
            ...
            InvokeUpdateEvent(render, true, true);
        }


        protected GameObject CreateItemGameObject(GameObject rawGameObject, Transform parent)
        {
            GameObject go = null;
            if (preCreateItemGameObject != null && preCreateItemGameObject.Count > 0 && preCreateItemGameObject[0] != null)
            {
                go = preCreateItemGameObject[0];
                preCreateItemGameObject.RemoveAt(0);
                if (parent != content)
            ...
            else if (!Application.isPlaying)
            {
                //预创建模式
#if UNITY_EDITOR
                UnityEditor.Selection.activeGameObject = m_Template;
            ...
            else
            {
                ...
                //    go = PreCreateCachePool.TryGetGameObject(m_TemplateAsset);

                if (go)
                    go.transform.SetParentOEx(parent);
                else
                    go = Instantiate<GameObject>(rawGameObject, parent, false);
            }
            return go;
        }


        void InvokeUpdateEvent(ListItemRenderer render, bool isLuaUpdate = false, bool addWait = true)
        {
            if (m_WaitCreateCount > 0 && addWait)
            {
                if (m_WaitFrameUpdateIndexList == null) m_WaitFrameUpdateIndexList = new List<int>();
                if (!m_WaitFrameUpdateIndexList.Contains(render.index))
                    m_WaitFrameUpdateIndexList.Add(render.index);
            }
            else
            {
                try
        ...
        protected void UpdateItemPosition(int index, Vector2 offset, ListItemRenderer render)
        {
            ...
            if (this.m_ItemSize != render.transform.sizeDelta && m_AnchorsAutoChange)
        ...
        public Vector2 GetItemPositionByIndex(int index)
        {
            ...
            if (index >= 0 && index < m_DataCount)
            {
                ...
                {
                    ListItemRenderer item;
                    if (this.m_ListItems != null && this.m_ListItems.TryGetValue(index, out item))
                    {
                        result = item.transform.localPosition;
                        result.x *= -1;
                        result.x += item.transform.rect.size.x * 0.5f;
                        result.y += item.transform.rect.size.y * 0.5f;
                ...
                else
                {
                    ...
                    if (m_GridConstraint == GridConstraint.ColumnCount)
                    {
                        int yIdx = Mathf.CeilToInt(index / hNum);
                        result.y = yIdx * m_ItemSize.y + yIdx * m_Spacing.y;
                    }
                    else if (m_GridConstraint == GridConstraint.RowCount)
                    {
                        int xIdx = Mathf.CeilToInt(index / vNum);
                        result.x = xIdx * m_ItemSize.x + xIdx * m_Spacing.x;
                        result.x = -result.x;
                    }
                }
            }
        ...
        public void ScrollToIndex(int index, float smoothTime = 0.1f, bool center = false, float offsetX = 0, float offsetY = 0)
        {
            if (!this.m_ScrollRect)
                return;
        ...
        //滑动列表让指定index的Item刚好显示出来
        public void ScrollToVisible(int index, float smoothTime = 0)
        {
            Vector2 contentPos = m_ScrollRect.content.anchoredPosition;
            float pos = Math.Abs(m_ListLayout == ListLayout.Horizontal ? contentPos.x : contentPos.y);
            float rectLength = m_ListLayout == ListLayout.Horizontal ? m_Viewport.rect.width : m_Viewport.rect.height;
            float itemLength = m_ListLayout == ListLayout.Horizontal ? m_ItemSize.x : m_ItemSize.y;
            float itemPos = itemLength * index + verticalSpacing * index;
            float diff = itemPos - pos;
            float offset = 0;
            if (diff < 0 || (diff > 0 && diff < itemLength)) //隐藏或部分隐藏于上（左）边缘
            {
                offset = -diff;
            }
            if (itemPos > pos + rectLength) //隐藏或部分隐藏于下（左）边缘
            {
                offset = itemPos - (pos + rectLength);
            }
            if (offset != 0)
            {
                Vector2 scrollPos = Vector2.zero;
                scrollPos.x = m_ListLayout == ListLayout.Horizontal ? contentPos.x + offset : contentPos.x;
                scrollPos.y = m_ListLayout == ListLayout.Horizontal ? contentPos.y : contentPos.y + offset;
                this.m_ScrollRect.ScrollToPosition(scrollPos, smoothTime);
            }
        }
        ...
        public class ListItemRenderer
        {
            ...
            public Vector3 recyclePos { get; set; }
