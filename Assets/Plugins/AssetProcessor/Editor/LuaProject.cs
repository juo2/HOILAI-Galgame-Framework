...
public class LuaProject : EditorWindow
{
    ...
    public static string LuaRootPath
    {
        get
        {
            if (string.IsNullOrEmpty(s_RootPath))
                s_RootPath = Path.Combine(Application.dataPath, "../A_Scripts/Lua/");
    ...
    void DoTreeView()
    {
        ...
        {
            BuildLuaWindow win = EditorWindow.GetWindow<BuildLuaWindow>();
            win.Show();
    ...
    {
        return Path.Combine(s_RootPath, "../LuaTemplateGen.exe");
    }
    ...
    class LuaProjectTreeView : TreeView
    {
        ...
        static Texture2D[] s_TestIcons =
        {
           ...
           (Texture2D)AssetDatabase.GetCachedIcon(Path.Combine(Path.GetDirectoryName(AssetManifest.s_AssetManifestPath), "Editor/temp.lua")),
        };
    ...
    class BuildLuaWindow : EditorWindow
    {
        GUIContent Style_RefershBtn;
        ...
        const string CLEARBUILDFOLDER = "BuildLuaWindow_CLEARBUILDFOLDER";
        const string c_buildTmpe = "../A_Build/";
        const string c_build2Tmpe = "../A_Build/{0}/00/";
        const string c_fileName = "devscript.lua";

        private int m_DeviceIdx = 0;
        private string[] m_Devices = new string[] { "None" };
        private BuildTarget m_BuildTarget = BuildTarget.Android;
        private string m_PackageName;
        private Compression m_Compression = Compression.LZMA;
        private string m_SdcardPath;
        private bool m_IsClearBuildFolder;
        private string m_BuildFolder;
        private string m_BuildCodePath;
        private bool m_IsRefreshing;
        private bool m_IsBuilding;
        private string m_AdbLog = "click build";
        void OnEnable()
        {
            Style_RefershBtn = new GUIContent((Texture)EditorGUIUtility.Load("d_RotateTool"), "device refresh");


            this.titleContent = new GUIContent("BuildLuaDevice");
            //this.maxSize = new Vector2(400, 230);
            this.m_IsClearBuildFolder = EditorPrefs.GetBool(CLEARBUILDFOLDER);
            this.m_PackageName = PlayerSettings.applicationIdentifier;
            this.m_SdcardPath = string.Format("/sdcard/Android/data/{0}/files/A_AssetBundles/{1}", this.m_PackageName, c_fileName);
            this.m_BuildFolder = Path.Combine(Application.dataPath, c_buildTmpe);
            this.m_BuildCodePath = Path.Combine(Application.dataPath, string.Format(c_build2Tmpe, XBuildUtility.GetPlatformAtBuildTarget(BuildTarget.Android)));
            RefershDevice();
        }
        ...
        {
            m_IsRefreshing = true;
            ADBUtility.GetDevices((string[] result) =>
            {
                if (result.Length > 0)
                {
                    m_Devices = result;
        ...
        void OnGUI()
        {
            EditorGUILayout.HelpBox("build lua to device", MessageType.Info);
            EditorGUILayout.BeginHorizontal();
            m_DeviceIdx = EditorGUILayout.Popup("Device", m_DeviceIdx, m_Devices);
            EditorGUI.BeginDisabledGroup(m_IsRefreshing);
            if (GUILayout.Button(Style_RefershBtn, "minibutton", GUILayout.Width(20), GUILayout.Height(15)))
                RefershDevice();
            EditorGUI.EndDisabledGroup();
            EditorGUILayout.EndHorizontal();
            m_BuildTarget = (BuildTarget)EditorGUILayout.EnumPopup("BuildTarget", m_BuildTarget);
            m_Compression = (Compression)EditorGUILayout.EnumPopup("Compression", m_Compression);
            m_PackageName = EditorGUILayout.TextField("PackageName", m_PackageName);
            EditorGUI.indentLevel++;
            {
                EditorGUI.BeginChangeCheck();
                this.m_IsClearBuildFolder = EditorGUILayout.Toggle("Clear Build Folder", this.m_IsClearBuildFolder);
                if (EditorGUI.EndChangeCheck())
                    EditorPrefs.SetBool(CLEARBUILDFOLDER, this.m_IsClearBuildFolder);

                Color scolor = GUI.contentColor;
                GUI.contentColor = Color.gray;
                EditorGUILayout.LabelField(s_RootPath);
                GUI.contentColor = scolor;

                EditorGUILayout.SelectableLabel(this.m_BuildFolder);
                EditorGUILayout.SelectableLabel(this.m_SdcardPath);
                EditorGUILayout.SelectableLabel(this.m_AdbLog);
            }
            ...
            {
                EditorUtility.OpenWithDefaultApp(this.m_BuildFolder);
            }
            ...
            if (GUILayout.Button("PushToDevice"))
                PushToDevice();
            EditorGUILayout.EndHorizontal();
        }
        ...
        {
            m_IsBuilding = true;
            BuildLuaParameter data = new BuildLuaParameter();
            data.outputPath = m_BuildFolder;
            data.isClearFolder = m_IsClearBuildFolder;
            data.buildTarget = m_BuildTarget;
            data.luaDirectory = LuaProject.LuaRootPath;
            data.fileName = c_fileName;
            data.buildAssetBundleOptions = BuildAssetBundleOptions.DisableLoadAssetByFileNameWithExtension;
            if (m_Compression == Compression.LZ4)
                data.buildAssetBundleOptions |= BuildAssetBundleOptions.ChunkBasedCompression;
            else if (m_Compression == Compression.LZMA)
                data.buildAssetBundleOptions |= BuildAssetBundleOptions.None;
            else
                data.buildAssetBundleOptions |= BuildAssetBundleOptions.UncompressedAssetBundle;

            XBuildLua.Build(data);
        ...
        void RemoveToDevice()
        {
            ADBUtility.RemoveFile(m_Devices[m_DeviceIdx], m_SdcardPath, (string data) =>
            {
                m_AdbLog = data;
                this.Focus();
            });
        ...
        void PushToDevice()
        {
            ADBUtility.PushFile(m_Devices[m_DeviceIdx], Path.Combine(m_BuildCodePath, c_fileName), m_SdcardPath, (string data) =>
            {
                m_IsBuilding = false;
                m_AdbLog = data;
                this.Focus();
